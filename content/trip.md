...
## Trip Service Overview
The Vinli device detects vehicle ignition and shutdown and sends those events to the platform. From these events, it is possible to organize the telemetry data into logical “Trips” for organizing users’ activities in your application. Trip Service provides access to a catalog of these trips by device or by vehicle and provides mirrors of the Telemetry Services methods centered around these trips.
...
#### Request
```curl
curl -u APP_ID:APP_SECRET -X GET "https://trips.vin.li/api/v1/devices/fe4bbc20-cc90-11e3-8e05-f3abac5b6b58/trips"
```
...
This method returns a list of all trips that a given vehicle has taken. This will include trips that have not yet been completed.
...
#### Request
```curl
curl -u APP_ID:APP_SECRET -X GET "https://trips.vin.li/api/v1/vehicles/0c785aa0-1a48-4cc6-9f5c-028350dd907d/trips"
```
...
#### Request
```curl
curl -u APP_ID:APP_SECRET -X GET "https://trips.vin.li/api/v1/trips/a51a3c87-baa7-4e5d-98e6-4f9588d7c2e1"
```
...
`fuelEconomy`|estimated fuel economy (in miles per gallon) during this trip. Uses gasoline as the default fuel type. However, diesel will be set as the fuel type (and corresponding diesel values for calculations) if it is detected in a trip.
...
### Substantial Trips
The `substantial` flag is used to help weed out trips that are too small/contain too little data to be considered valuable for most use cases. These "tiny" trips are generated from real life scenarios, like moving a vehicle from one side of the driveway to another. By default Trip Service will only return substantial trips but in the event that you need all trips, including insubstantial ones, you can add the `substantialOnly=false` query parameter to your request.